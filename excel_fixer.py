...
PDF_ROW_MAP = {
    5: 'Revenus mensuels',
    6: 'Revenus Journaliers',
    7: 'Revenus Lave-Auto',
    8: 'Divers',
    10: 'Gratuités - mensuels',
    11: 'TOTAL REVENUS',
    14: 'Salaires Stationnement',
    15: 'Uniformes',
    16: 'Fourn. de stationnement',
    17: 'Entretien réparation - Nettoyage',
    18: 'Entretien réparation - Equipement',
    19: 'Entretien réparation - Général',
    20: 'Taxes et permis',
    21: 'Assurances Cautionnement',
    22: 'Réclamations',
    23: 'Télécommunication',
    24: 'Frais de cartes de crédit',
    25: 'Frais de bureau',
    26: "Total des frais d'exploitation",
    27: "RÉSULTAT D'EXPLOITATION",
    29: 'Honoraires de gestion',
    31: 'BÉNÉFICE NET',
}
...
def extract_from_digital_pdf(pdf_path):
    ...
    for row_num, account_name in PDF_ROW_MAP.items():
        if row_num <= len(table):
            ...
            if amount is not None: data[account_name] = amount
    return data
...
def extract_from_image_pdf(pdf_path):
    ...
    for line in lines:
        ...
        if len(line_lower) < 5: continue
        for account_name, keywords in account_keywords.items():
            if account_name in data: continue
            for kw in keywords:
                if kw in line_lower:
                    number_patterns = [
